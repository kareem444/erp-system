...
const useCrudHandler = <T extends { id?: number }>(asyncDataKey: OriginalItemsKeyType) => {
    const { setState } = useAsyncState<T[]>(asyncDataKey);
    const { state: searchedCustomer, setState: setSearchedCustomers } = useEchoState<T[]>(EchoStateConstants.searchedItems);

    const createOperation = (newItem: T) => {
        setState(prevState => {
            return {
        ...
        if (searchedCustomer && searchedCustomer.length > 0) {
            setSearchedCustomers([...searchedCustomer, newItem])
        }
    }
    ...
    const updateOperation = (updatedItem: T) => {
        ...
        if (searchedCustomer && searchedCustomer.length > 0) {
            setSearchedCustomers(searchedCustomer.map(item => {
                if (item.id === updatedItem.id) {
                    return { ...item, ...updatedItem }
                }
                return item
    ...
    const deleteOperation = (deletedItem: T) => {
        ...
        if (searchedCustomer && searchedCustomer.length > 0) {
            setSearchedCustomers(searchedCustomer.filter((item) => item.id !== deletedItem.id))
        }
    }
